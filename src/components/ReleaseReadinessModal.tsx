      ...
      case 'failed':
        return (
          <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[11px] font-bold bg-rose-50 text-rose-700 border border-rose-200">
            <XCircle className="w-3 h-3 text-rose-600" /> Block
          </span>
        );
    }
  };
                  ...
                  return (
                    <button
                      ...
                      className={`p-3 rounded-xl border text-left transition-all cursor-pointer relative overflow-hidden flex flex-col justify-between ${
                        isSelected
                          ? 'bg-purple-50/80 border-purple-500 ring-2 ring-purple-400/30 shadow-xs'
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <h4 className="text-sm font-bold text-slate-900 flex items-center gap-2">
                      ...
                      <span className="text-[10px] font-mono text-slate-500">Weight: {activePillarData.weight * 100}%</span>
                    </h4>
                    <p className="text-xs text-slate-500">{activePillarData.target}</p>
                  </div>
                </div>

                <div className="flex items-center gap-2">
                  <span className="text-xs font-mono font-bold text-slate-700">Pillar Score:</span>
                  <span className={`text-xs font-extrabold px-2.5 py-1 rounded-lg border ${
                    activePillarData.status === 'passed' ? 'bg-emerald-100 text-emerald-800 border-emerald-300' :
                    activePillarData.status === 'warning' ? 'bg-amber-100 text-amber-800 border-amber-300' :
                ...
                {report.blockers.length > 0 ? (
                  <div className="space-y-2">
                    {report.blockers.map((b, i) => (
                      <div
                        key={i}
                        className="p-2.5 rounded-lg bg-rose-50 border border-rose-200/80 text-xs text-rose-900 flex items-start justify-between gap-2"
                      >
                        ...
                        {onRemediateBlocker && (
                          <button
                            ...
                            className="text-[10px] font-bold text-purple-700 hover:text-purple-900 hover:underline shrink-0 bg-white px-2 py-1 rounded border border-rose-200"
                          >
                            Fix with Byte →
                          </button>
                        )}
                  ...
                  {report.signOffChecklist.map((item) => (
                    <div
                      ...
                      className={`p-2 rounded-lg border text-xs flex items-center justify-between gap-2 ${
                        item.passed ? 'bg-emerald-50/60 border-emerald-200/60 text-slate-800' : 'bg-slate-50 border-slate-200 text-slate-600'
                      }`}
                    >
                      <div className="flex items-center gap-2">
                        {item.passed ? (
                        ...
                        <span className={`text-[11px] ${item.passed ? 'font-semibold text-emerald-950' : 'text-slate-600'}`}>
                          {item.label}
                        </span>
                      </div>
          ...
          <div className="p-4 sm:p-5 bg-slate-50 border-t border-slate-200 flex items-center justify-between gap-3 flex-wrap">
            <div className="flex items-center gap-2">
              <button
                ...
                className="px-3 py-2 rounded-xl bg-white hover:bg-slate-100 text-slate-700 border border-slate-200 text-xs font-bold flex items-center gap-1.5 transition-colors cursor-pointer"
              >
                {copiedSummary ? <Check className="w-3.5 h-3.5 text-emerald-600" /> : <ClipboardCheck className="w-3.5 h-3.5 text-slate-500" />}
